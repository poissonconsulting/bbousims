test_that("bbs_survival works", {
  ...
  expect_equal(dim(x), c(4, 3, 2))
  expect_true(all(x[,,1] == ilogit(logit(b0_1))))
  expect_true(all(x[,,2] == ilogit(logit(b0_2))))
})
...
test_that("bbs_survival with trend works", {
  ...
  expect_true(all(x[,1,1] == ilogit(logit(b0_1))))
  expect_true(all(x[,1,2] == ilogit(logit(b0_2))))
  # increasing within stage
  expect_true(all(x[1, , 1] >= x[1, 1, 1]))
  expect_true(all(x[1, , 2] >= x[1, 1, 2]))
  # stage 2 higher than stage 1 
  expect_true(all(x[1, , 2] >= x[1, , 1]))
})
...
test_that("bbs_survival with rndm annual works", {
  ...
  expect_false(all(x[,1,1] == ilogit(logit(b0_1))))
  # first month not equal to intercept
  expect_false(all(x[1,,2] == ilogit(logit(b0_2))))
  # years not equal stage 1
  expect_equal(length(unique(x[1, , 1])), 3)
  # month not equal stage 2
  expect_equal(length(unique(x[, 1, 2])), 4)
  # within month equal stage 1
  expect_equal(length(unique(x[1, , 2])), 1)
  # within year equal stage 2
  expect_equal(length(unique(x[,1 , 1])), 1)
})
...
test_that("bbs_survival with rndm period and annual works", {
  ...
  expect_true(all(unlist(x) >= 0) & all(unlist(x) <= 1))
  # first period not equal to intercept
  expect_false(all(x[1,,1] == ilogit(logit(b0_1))))
  # first year not equal to intercept
  expect_false(all(x[,1,2] == ilogit(logit(b0_2))))
  # periods and years not equal
  expect_equal(length(unique(x[, 1, 1])), 4)
  expect_equal(length(unique(x[1, , 1])), 3)
  expect_equal(length(unique(x[, 1, 2])), 4)
  expect_equal(length(unique(x[1, , 2])), 3)
})
...
test_that("bbs_survival_caribou works with trend", {
  ...
  expect_true(all(as.vector(x[, 1, 3]) == ilogit(logit(survival_adult_female^(1/12)))))
  expect_true(all(as.vector(x[, 1, 2]) == ilogit(logit(survival_adult_female^(1/12)) + yearling_effect)))
  expect_true(all(as.vector(x[, 1, 1]) == ilogit(logit(survival_calf_female^(1/12)))))
  expect_true(all(x[1,,1] >= ilogit(logit(survival_calf_female^(1/12)))))
  expect_equal(length(unique(x[1,,1])), nyear)
  expect_true(all(x[1,,3] >= ilogit(logit(survival_adult_female^(1/12)))))
  expect_equal(length(unique(x[1,,3])), nyear)
})
...
test_that("bbs_survival_caribou works with random effects", {
  ...
  expect_equal(length(unique(x[, 1, 2])), 1)
  expect_equal(length(unique(x[, 1, 3])), 1)
  
  # linear relationship logit yearling and adult
  expect_equal(length(unique(as.vector(round(logit(x[,,2]) - logit(x[,,3]), 2)))), 1)
})
