test_that("fecundity process matrix", {
  rates <- c(0, 0, 0.9)
  x <- matrix_birth(rates, female_recruit_stage = 1, male_recruit_stage = NULL)
...
test_that("fecundity process matrices", {
  ...
  x <- matrix_birth_year(rates, male_recruit_stage = NULL)
...
test_that("fecundity stochastic", {
  ...
  x <- fecundity_year(calves_per_adult_female = calves_per_adult_female,
