...
The ability of different models to recover the 'true' annual survival and recruitment rates can be assessed. 
...
For example, the true annual adult female survival rates can be calculated from the projected population as the product of the monthly survival rates for each year
```{r}
...
fits_survival <- map(1:nsims, function(x){
  ...
  bboutools::bb_fit_survival(data = survival, 
                             year_start = 1L, 
...
all(
  unlist(
    x <- map(fits_survival, function(x){
      glance(x)
    })
  )
)
...
preds <- map_df(1:length(fits_survival), function(x){
...
preds <- 
  ...
  summarize(med = median(estimate), 
            ...
            upper = quantile(estimate, 0.0975)) %>%
  ungroup()
...
actual <- tibble(CaribouYear = 1:nyear, actual = saf_annual)
 ...
 gp <- 
    ggplot(data = preds) +
    geom_pointrange(aes(x = CaribouYear, y = estimate, ymin = lower, ymax = upper)) +
    xlab("Year") +
    scale_y_continuous("Annual Survival") +
    geom_point(data = actual, aes(x = CaribouYear, y = actual), color = "red") 
 gp
```

The true recruitment rates can be calculated from the projected population at each composition survey month as 
```{r}
month_composition = 9L
...
# decesare adjusted recruitment to match bboutools predictions
fcaf <- (caf * 0.5) 
recruitment <- (fcaf / (1 + fcaf))

actual_recruitment <- tibble(Year = 1:length(recruitment),
                             actual = recruitment)
```
...
fits_recruitment <- map(1:nsims, function(x){
  recruitments <- data[[x]]$recruitments
  bboutools::bb_fit_recruitments(data = recruitment, 
                             year_start = 1L, 
                             quiet = TRUE, 
                             nthin = 100, 
                             niters = 500)
...
all(
  unlist(
    x <- map(fits_recruitment, function(x){
      glance(x)
    })
  )
)
...
# predict annual survival
preds <- map_df(1:length(fits_recruitment), function(x){
  pred <- bboutools::bb_predict_survival(fits_recruitment[[x]])
  pred$sim <- x
  pred
})
